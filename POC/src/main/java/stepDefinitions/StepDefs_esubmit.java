...
public class StepDefs_esubmit {
	...
	public void i_submit_the_request() throws Throwable {
		System.out.println("Setting request to be submited" + request);
		DBTasks dbTasks = new DBTasks();
		dbTasks.GetDataForRequest();
		System.out.println(dbTasks.GetDataForRequest().toString());

	}

	@Then("^The result should <returnResult> <text>$")
	public void the_result_should_returnresult_text(DataTable returnResult) throws SQLException, IOException {
		DBTasks dbtasks = new DBTasks();
		lynxxdata = new LynxxData(requistionId, customerId, veterinarian, collectionDate, clinicalDetails, patientDOB, testCode);
		String http = "http://lnx-us-pre-order-service-dev.appspot.com/ESubmit/1.0.0/orders/" + requestID; // should
																											// this be
																											// in a
																											// definition
																											// ?
		...
		dbtasks.saveRequestDatatoDB(lynxxdata.requisitionId,customerId,veterinarian,clinicalDetails, collectionDate, patientDOB,testCode);
	...
	//
	// String
	// http="http://lnx-us-pre-order-service-dev.appspot.com/ESubmit/1.0.0/orders/";
	// //should this be in a definition ?
	//
	// String Data = dbTasks.GetDataForRequest();
	// //"{\"requisitionId\": \"VCP012397\", \"customerId\": \"G700, ISTI, 39488\",
	// \"veterinarianv\": \"DR SMITH\"}";
	// connection("");
	...
	// connection("");
	// String
	// http="http://lnx-us-pre-order-service-dev.appspot.com/ESubmit/1.0.0/orders/VPC0000309";
	// //should this be in a definition ?
	//
	// String Data = "{\"requisitionId\": \"VCP0000309\", \"customerId\": \"G700,
	// ISTI, 39488\", \"veterinarian\": \"DR SMITH\"}";
	//
	// URL url = new URL(http);
	// HttpURLConnection connection = (HttpURLConnection) url.openConnection();
	//
